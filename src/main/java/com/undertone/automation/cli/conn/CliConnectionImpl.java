...
import com.undertone.automation.assertion.AbstractAssertionLogic;
import com.undertone.automation.assertion.IAssertionLogic;
import com.undertone.automation.cli.terminal.BufferInputStream;
import com.undertone.automation.cli.terminal.Cli;
import com.undertone.automation.cli.terminal.DummyTerminal;
import com.undertone.automation.cli.terminal.InOutInputStream;
import com.undertone.automation.cli.terminal.Prompt;
import com.undertone.automation.cli.terminal.SSH;
import com.undertone.automation.cli.terminal.SSHWithRSA;
import com.undertone.automation.cli.terminal.Telnet;
import com.undertone.automation.cli.terminal.Terminal;
...
public abstract class CliConnectionImpl extends AbstractModuleImpl<String> implements CliConnection {
	...
	synchronized (this) {
		...
		for (int retriesCounter = 0; retriesCounter < connectRetries; retriesCounter++) {
			...
			Reporter.error(
				"Connecting to " + getHost() + " Failed. (Attempt #" + (retriesCounter + 1) + ")", e);
			try {
			    disconnect();
			} catch (Throwable t) {
			    Reporter.error("Disconnecting to " + getHost() + " Failed.", t);
			}
	...
	if (dummy) {
	    Reporter.info("Connecting to a dummy CLI ...");
	...
	Reporter.info("Connecting to [" + host + "] ...");
	// Terminal t;
	...
	if (idleMonitor != null) {
		...
		Reporter.error("Waiting for idle monitor failed", e);
	    }
	}
	disconnect();
	isClosed = true;
     ...
     * 3. Performs report operation & throws an exception in case of an
     * error.<br>
     ...
     */
    public static void handleCliCommand(CliConnectionImpl cli, String title, CliCommand command)
		...
		if (command.isIgnoreErrors() || (cli.isForceIgnoreAnyErrors())) {
			...
			for (IAssertionLogic<String> analyzer : analyzers) {
				...
				if (!(command.isSilent() && stringAssertionLogic.isStatus())) {
					...
					cli.Reporter.info(
						stringAssertionLogic.getTitle() + stringAssertionLogic.getMessage());
				    } else {
					cli.Reporter.error(
						stringAssertionLogic.getTitle() + stringAssertionLogic.getMessage());
				    }
	...
	if (idleMonitor != null) {
		...
		try {
		    ...
		    Reporter.error("Deactivating idle monitor failed", e);
		} finally {
		    idleMonitor = null;
		}
	    }
	...
	return Optional.ofNullable(super.getName()).orElse(getHost());
    }
