...
import com.undertone.automation.cli.terminal.Cmd;
import com.undertone.automation.cli.terminal.Prompt;

/**
 * Implementation of the CliConnection interface for working with windows
 * CMD.exe application directly. When activating some command line applications
 * (like wmic), the application doesn't get the output unless the output stream
 * is closed. To support such applications, the
 * <code>cloneOnEveryOperation</code> parameter was added tho this class. When
 * this parameter is set to true, the output stream of the connection is closed
 * on every operation. The outcome of this is that a new process is created for
 * every command that the user activates. The user cn also set isClone()
 * parameter of the CliCommand object. This will have the same affect but only
 * for a single CliCommand.
 * 
 * @author goland
 */
