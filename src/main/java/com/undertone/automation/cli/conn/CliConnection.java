...
public interface CliConnection {
     ...
     * 
     * @param notifications
     *            the array of strings to wait for in the result
     * @param timeout
     *            time in ms after which the command fails
     * @throws Exception
     */
    ...
    /**
     * checks if the connection should be established when initiating the
     * SystemObject<br>
    ...
    /**
     * If set to True add enter after waitForPromp fail on timeout And will wait
     * again for prompt
     */
    public void setGraceful(boolean graceful);

    /**
     * Flag that indicates if after command timeout an enterString should be
     * sent
     * 
     * @return
     */
    ...
    /**
     * Activates the command <code>command</code> and analyzes command's results
     * Procedure flow: 1. connects to remote machine if needed<br>
     ...
     * 4. Performs Analysis if one or more analyzers are defined (and ignore
     * error flags were not raised)
     * 
     * @throws InterruptedException
     */
     ...
     *            the command to send
     * @param delayedTyping
     *            if True will sleep 20 ms between each typed byte entered to
     *            the terminal
     * @throws Exception
     */
