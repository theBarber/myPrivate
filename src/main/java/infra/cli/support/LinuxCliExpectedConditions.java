...
import static infra.cli.support.CliExecutionExpectedConditions.executionLogicHappens;
import static infra.support.Parsers.intParser;
...
import infra.assertion.ComparableAssertion;
...
import infra.support.CompareMethod;
...
public abstract class LinuxCliExpectedConditions {
	...
	return new Predicate<LinuxDefaultCliConnection>() {
	    ...
	    @Override
	    public boolean test(LinuxDefaultCliConnection cliConnection) {
		try {
	...
	return new CliExpectedCondition<String>() {
		...
		try {
		    return linux.fileList(directory)
			    .filter(currFileName -> currFileName.substring(directory.length()).equals(file)).findFirst()
			    .orElse(null);
		} catch (FileNotFoundException e) {
		    throw null;
		} catch (IllegalArgumentException e) {
     ...
     */
    public static CliExpectedCondition<String> directoryContains(final String directory, final String file,
	...
	return new CliExpectedCondition<String>() {
	    ...
	    public String apply(LinuxDefaultCliConnection linux) {
		try {
		    return linux.fileList(directory).filter(currFileName -> currFileName.matches(file)).findFirst()
			    .orElse(null);
		} catch (FileNotFoundException e) {
		    return null;
		} catch (IllegalArgumentException e) {
    ...
    public static CliExpectedCondition<Integer> directoryHas(final String directory, final String filenamePattern,
	    final CompareMethod compareMethod, final int expectedAmount, final DirectoryItemType itemType) {

	StringBuilder commandBuilder = new StringBuilder().append("find ").append(directory)
		.append(" -maxdepth 1 -type ").append(itemType.typeCmd).append(" -name ").append(filenamePattern)
		.append(" | wc -l");
	final String findCommand = commandBuilder.toString();
	final ComparableAssertion<Integer> amountOfItemsAssertion = new ComparableAssertion<>(compareMethod,
		expectedAmount);
	return new CliExpectedCondition<Integer>() {

	    @Override
	    public Integer apply(CliConnection cliConnection) {
		return executionLogicHappens(amountOfItemsAssertion, intParser)
			.apply(new CliCommandExecution(cliConnection, findCommand).silently());
	    }

	    @Override
	    public String toString() {
		return "the directory's " + directory + " amount of " + itemType.name() + "(s) to be "
			+ compareMethod.toString() + " " + expectedAmount;
	    }
	};
    }
