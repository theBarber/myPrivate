...
import infra.support.Formatter;
import infra.support.Parser;
...
public class LinuxFileInfoExecutionBuilder {
	...
	return new CliCommandExecution(cliConnection, "ls -l --time-style=full-iso " + this.name
		+ " | grep -v '^total' | awk {'print " + resultType.awkPrint + "'}").error("No such file or directory")
			.error("cannot access");
    }
    ...
    // public CliExecutionExpectedCondition<Long> fileSizeToBe(CompareMethod
    // compareMethod, long expectedSize) {
    // getFileSize();
    // return execute(resultType, expectedSize, compareMethod);
    // }
    //
    // private CliExecutionExpectedCondition<Long> execute(final ResultType
    // resultType, final long expected,
    ...
    // new ComparableAssertion<>(compareMethod,
    // expected).examinedObjectTitled(examinedObjectName.toString())
