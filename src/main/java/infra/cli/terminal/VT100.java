...
class ScreenReader extends Thread {
	...
	try {
		...
		case 0x1B:// 'ESC'
			...
			if (c5 != ';') {
			    ...
			    System.err
				    .println("There could be a problem c5: " + (char) c5 + " " + (char) c3 + (char) c4);
			}
			int c6 = in.read();
			int c7 = in.read();
			int c8 = in.read();
			...
			currentRow = Integer.parseInt(new String(new char[] { (char) c3, (char) c4 })) - 1;
			currentColumn = Integer.parseInt(new String(new char[] { (char) c6, (char) c7 })) - 1;
			break;
     ...
     * + buf.toString(), screen, true); setTestAgainstObject(screen);
     * analyze(new TextNotFound("ERROR"), true); }
     * 
     */
