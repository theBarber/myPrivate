...
public class Prompt {
     ...
     * @param regexCaseInsensitive
     */
    public Prompt(String prompt, boolean regularExpression, boolean dontWaitForScrollEnd, boolean commandEnd,
	    String stringToSend, boolean addEnter, boolean regexCaseInsensitive) {
	super();
	setPrompt(prompt);
	...
	if (pattern == null) {
	    pattern = Pattern.compile(prompt,
		    Pattern.DOTALL | (isRegexCaseInsensitive() ? Pattern.CASE_INSENSITIVE : 0));
	}
	return pattern;
    }
