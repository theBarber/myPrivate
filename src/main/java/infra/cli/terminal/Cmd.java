...
/**
 * Terminal implementation for working on top of windows's cmd.exe terminal
 * emulation.
 * 
 * When set to true the output stream of the cmd.exe process is closed after
 * string is sent. This was added for applications that require closing the
 * output stream in order for the data to get to their input stream. An example
 * application is wmi command line tool (wmic)
 * 
...
public class Cmd extends Terminal {
	...
	File root = Optional.ofNullable(processDir).map(File::new).orElse(null);
    ...
    @Override
    public synchronized void sendString(String command, boolean delayedTyping)
