...
public class API_EntitiesCreator extends BaseTest {
    ...
    private void updateCampaignEndDate(String campaign_name, Integer days) {
        ...
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        //************ check if UTC & EST are in the same day ******************
        Calendar utc = Calendar.getInstance();
        Calendar est = Calendar.getInstance();
        TimeZone utcTimeZone = TimeZone.getTimeZone("UTC");
        utc.setTimeZone(utcTimeZone);
        est.setTimeZone(etTimeZone);
        int dayEst = est.get(Calendar.DAY_OF_MONTH);
        int dayUtc = utc.get(Calendar.DAY_OF_MONTH);
        int deFactoDays = (dayUtc > dayEst) ? days : days - 1;
    ...
    private void updateEntityDataByID(String entity, String updateBy, DataTable entities) {
        ...
        for (int i = 1; i < EntityList.size(); i++) {
            ...
            for (int j = 1; j < entityData.size(); j++) {
                if (entityData.get(j).equalsIgnoreCase("NULL")) {
                    SqlWorkflowUtils.setNullValueInWf(entity + "s", entity + suffix, entityID.toString(), EntityList.get(0).get(j));
                } else {
