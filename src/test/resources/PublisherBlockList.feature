...
Feature: Publisher blacklist tests
  ...
  Scenario: For a publisher 3843 block advertiser 22420 - dynamic tag request
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3843 with extra params {notblocked.com&tagid=470&optimize=0}
    And The response code is 200
    And The response contains script
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-pbl-BRAND1-ES-banner-1} 100% of the time
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3843 with extra params {blocked.com&tagid=470&optimize=0}
    And The response code is 200
    And The responses are passback
