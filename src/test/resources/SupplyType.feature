...
Feature: Supply type flow support
  ...
  Scenario: verify when HB first
    Given i send 1 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [1,2]} for publisher 3719 with domain {test.com} with extra params {&unlimited=1&optimize=0}
  ...
  Scenario: verify when Direct first
    Given i send 1 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [1,1]} for publisher 3719 with domain {test.com} with extra params {&unlimited=1&optimize=0}
    And The response code is 200
    And The response contains script
  ...
  Scenario: verify when All first
    Given i send 1 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [300,250]} for publisher 3719 with domain {test.com} with extra params {&unlimited=1&optimize=0}
    And The response code is 200
    And The response contains script
