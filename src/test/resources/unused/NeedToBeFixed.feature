...
Feature: Need To Fix support
  ...
  Scenario: 1. a. run on false domain flag = true. req sent from a false domain - zone req, DT req.
    When I send 1 times an ad request with parameter {unlimited=1&optimize=1&domain=daniellafalsedomain.test} for zone named {zone-zoneset-YesRunFalseDomain-69-1} to UAS
    Then The response code is 200
    And The response contains {script}
  ...
  Scenario: 7A. send DT requests, referer is a false domain, should choose domain as effective host.
    ...
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3739 with extra params {danidom.com&tagid=470&optimize=1}
    ...
    Given I clear all headers from uas requests
    Given I clear all cookies from uas requests
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3739 with extra params {news.danidom.com&tagid=470&optimize=1}
    And The response code is 200
    And The response contains {script}
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-EHC-DT-SS-7A-banner-1} 100% of the time
