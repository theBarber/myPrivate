...
Feature: NDQ Filtering Campaign Level
  ...
  Scenario: 1. Campaign level, zone request, verify NDQ filtering obeys to skip daily goal flag when flag = false
    And I send 100 times an ad request with query parameters for zone named {zone-zoneset-NDQfiltering2CL-ST-1} to UAS
    And The response code is 200
    And The responses has impression-urls
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-NDQfiltering2CL-ST-1-banner-1} 100% of the time
    And I send 5 times an ad request with query parameters for zone named {zone-zoneset-NDQfiltering2CL-ST-1} to UAS
    And The response code is 200
  ...
  Scenario: 3 Campaign level, HB request, verify NDQ filtering obeys to skip daily goal flag when flag = false
    ...
    And The response code is 200
    And The response contains script
    And all HB responses contains adId with id of entity named {campaign-NDQfiltering2CL-ST-3-banner-1}

  Scenario: 4 Campaign level, zone request, verify NDQ filtering obeys to skip daily goal flag when flag = true
    And I send 110 times an ad request with query parameters for zone named {zone-zoneset-NDQfiltering2CL-ST-4} to UAS
    And The response code is 200
    And The responses has impression-urls
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-NDQfiltering2CL-ST-4-banner-1} 100% of the time
  ...
  Scenario: 5 Campaign level, DT request, verify NDQ filtering obeys to skip daily goal flag when flag = true
    ...
    And The response contains script
    And The responses has impression-urls
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-NDQfiltering2CL-ST-5-banner-1} 100% of the time
    Given I use {http://NDQfiltering} as referer string to send my requests to uas
  ...
  Scenario: 6 Campaign level, HB request, verify NDQ filtering obeys to skip daily goal flag when flag = false
    ...
    And The response code is 200
    And The response contains script
    And all HB responses contains adId with id of entity named {campaign-NDQfiltering2CL-ST-6-banner-1}
