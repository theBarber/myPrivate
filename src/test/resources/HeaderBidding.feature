...
#---frequency capping
  ...
  Scenario: header bidding frequency capping from mobile - user 1
    ...
    Given i send 20 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [1,1],[1,2]} for publisher 3719 with domain {testing.com} with extra params {&unlimited=1&optimize=0}
    And The response code is 200
    And The response contains script
    And i read all HB responses and map their bidId by adId
    ...
    Given i send 1 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [1,1],[1,2]} for publisher 3719 with domain {testing.com} with extra params {&unlimited=1&optimize=0}
  ...
  Scenario: header bidding frequency capping from Desktop user 1
    ...
    Given i send 30 headerBidding post request for scenario {Send HB basic request for publisher 3719 size [1,1],[1,2]} for publisher 3719 with domain {testing.com} with extra params {&unlimited=1&optimize=0}
    And The response code is 200
    And The response contains script
    And i read all HB responses and map their bidId by adId
    And in HB responses bidid bid1 has entity of adId with name {campaign-HB-Tablet-160x600-banner-1} 50% of the times
