...
#    Given I clear all cookies from uas requests
#    Given i send 1 headerBidding secure post request for publisher 3728 with multi sizes - h1:1 w1:1, h2:1 w2:2 with domain {slader.com} and placmentID group = {3728001} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us}
#    And The response code is 200
#    And The response contains {script}
...
#    And all HB responses contains cpm with id 3
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - pg disabled throttling 0%
    Given I clear all cookies from uas requests
    Given i send synchronized 10 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728010} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us} cookies false
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 0%
    Given I clear all cookies from uas requests
    Given i send synchronized 10 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728011} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us} cookies false
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 60%
    Given I clear all cookies from uas requests
    Given i send synchronized 100 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728001} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us} cookies false
...
##----------------------------------------------------------------------------------------------------------------------------------------------
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 70%
    ...
    Given i send synchronized 100 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728011} and extra params {&deviceid=throttling1&optimize=1&unlimited=1&sim_geo=1&country=us} cookies true
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 50%
    Given I clear all cookies from uas requests
    Given I add cookie UTID with random value to my requests to uas
    Given i send synchronized 100 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728011} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us} cookies true
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 40%
    Given I clear all cookies from uas requests
    Given i send synchronized 100 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728001} and extra params {&deviceid=throttling1234&optimize=1&unlimited=1&sim_geo=1&country=us} cookies false
  ...
  Scenario: 1. 2 sizes - 1*1 (SS, PG), 1 placment (PG), PG banner expected - throttling 100%
    Given I clear all cookies from uas requests
    Given I add cookie UTID with random value to my requests to uas
    Given i send synchronized 100 basic headerBidding secure post request for publisher 3728 with size - h1:1 w1:1, with domain {slader.com}, placmentID group = {3728001} and extra params {&optimize=1&unlimited=1&sim_geo=1&country=us} cookies true
    Then The synchronized response code is 200 10 of the times
