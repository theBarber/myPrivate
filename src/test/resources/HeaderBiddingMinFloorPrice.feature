...
#***** PUBLISHER 2434 - placement group id = 2434010  ****
  ...
  Scenario: 3.1 - Direct - beyond floor price - 4.98$
    Given i send 1 headerBidding post request for scenario {Send HB getting 4.98 price} for publisher 2434 with domain {Dynamic1.com} with extra params {&optimize=1}
    And The response code is 200
    ...
    And all HB responses contains cpm with value {4.98}


  Scenario: 3.2 - Direct - below floor price - passback?
    Given i send 1 headerBidding post request for scenario {Send HB getting below floor price} for publisher 2434 with domain {Dynamic2.com} with extra params {&optimize=1}
    And The response code is 204