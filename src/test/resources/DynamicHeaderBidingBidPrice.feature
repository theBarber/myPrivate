...
Feature: Dynamic HB bid price
  ...
  Scenario: 4. campaign with dynamic bid price, campign desktop > po line item > campaign mobile
    ...
    And The response contains script
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-HB-DynamicPrice-3-banner-1} 100% of the time
    And For bidID 14 The field CPM in the 17 column of the hbl log is: 7.00
    ...
    And For bidID 15 The field CPM in the 17 column of the hbl log is: 7.00


  Scenario: 5. campaign with dynamic bid price for adsize 1*2 = 9, fixed price for adsize 300*250(po price = 2)
    ...
    And The response code is 200
    And The response contains script
    And all HB responses contains campaignId with id of entity named {campaign-HB-DynamicPrice-5}
    And all HB responses contains adId with id of entity named {campaign-HB-DynamicPrice-5-banner-1}
    And all HB responses contains cpm with value {0.6}
    And for all HB responses i simulate winning, and send their zone tag
    And The response code is 200
    And The response contains script
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-HB-DynamicPrice-??????-banner-1} 100% of the time
    And all HB responses contains cpm with value {0.6}
    ...
    And The response code is 200
    And The response contains script
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-HB-DynamicPrice-6-banner-1} 100% of the time
    And all HB responses contains cpm with value {0.3}
