...
Feature: Validate New Render Service
  ...
  Scenario: Send a web request to a zone linked to no banners and validate Passback
    ...
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-Passback} to UAS
  ...
  Scenario: Send a web zone request and validate the response of a direct PG banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-Direct-Zone-Regular} to UAS
    Then The response code is 200
  ...
  Scenario: Send a web DT request and validate the response of a direct PG banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3728 with domain {12news.com&tagid=238&optimize=1}
  ...
  Scenario: Send a web HB request and validate the response of a direct ES banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    Given i send 1 headerBidding secure post request for scenario {Send HB PPP request for publisher 3673 with 300X250} for publisher 3673 with domain {americanow.com} with extra params {&unlimited=1&optimize=1&sim_geo=1&country=us}
    And The response code is 200
...
#    And The response contains {var ut_scripts = ["https://pixel.adsafeprotected.com/jload?anId=}
  ...
  Scenario: Send a web HB request and validate the response of a Programmatic ES banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    Given i send 1 headerBidding secure post request for scenario {Send HB PPP request for publisher 3728 with 300X250} for publisher 3728 with domain {testing.com} with extra params {&requestid=OXESRender&unlimited=1&optimize=1&sim_geo=1&country=us}
    And The response code is 200
    And The response contains {script}
  ...
  Scenario: Send a web DT request and validate the response of a programmatic PG banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3728 with domain {damndelicious.net&requestid=PGZoneRender&tagid=238}
    And The response code is 200
    And The response contains {ut_udms}
    And The responses has impression-urls
  ...
  Scenario: Send a web zone request and validate the response of a direct PG Action banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-Direct-Zone-PGAction} to UAS
    Then The response code is 200
    And The response contains {ut_scripts}
    And The response contains {"https://pixel.adsafeprotected.com/jload?anId=}
  ...
  Scenario: Send a web DT request and validate the response of a direct PG Action banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3673 with domain {chess24.com&tagid=349}
    And The response code is 200
    And The response contains {ut_scripts}
    And The response contains {"https://pixel.adsafeprotected.com/jload?anId=}
    And The response contains {renderTriggers}
  ...
  Scenario: Send a web zone request and validate the response of a programmatic PG Action banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {requestid=AN-PGA-Render&unlimited=1} for zone named {zone-zoneset-NewRenderTest-Prog-Zone-PGAction} to UAS
    Then The response code is 200
#    And The response contains {ut_scripts}
#    And The response contains {"https://pixel.adsafeprotected.com/jload?anId=}
...
#    And The response contains {'exit_viewport\', \'back_button\'}
  ...
  Scenario: Send a web DT request and validate the response of a programmatic PG Action banner
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3673 with domain {gamesradar.com&requestid=AN-PGA-Render&tagid=617}
    And The response code is 200
#    And The response contains {ut_scripts}
#    And The response contains {"https://pixel.adsafeprotected.com/jload?anId=}
    And The response contains {window.ut_renderTriggers=[1]}
#    And The response contains {'exit_viewport\', \'back_button\'}
  ...
  Scenario: Send a web zone request from US and validate the response of a Direct Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-web-direct-zone-video} to UAS
    Then The response code is 200
  ...
  Scenario: Send a web DT request and validate the response of a Direct Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with domain {autoweek.com&tagid=430&optimize=1}
    And The response code is 200
    And The response contains {var ut_aniview={publisherId:\'5912cb1028a06142db67f38a\',channelId:\'5b7d30c4073ef454a07d4d64\'}}
    And The response contains {VASTAdTagURI}
    And The response contains {ut_udms}
  ...
  Scenario: Send a web HB request and validate the response of a Direct Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    Given i send 1 headerBidding secure post request for scenario {Send HB request for publisher 3708 with 320X50} for publisher 3708 with domain {autoshopper.com} with extra params {&unlimited=1&optimize=1&sim_geo=1&country=us}
    And The response code is 200
    And The response contains {script}
...
#    And The firstQuartileUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The midpointUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The thirdQuartileUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The completeUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The pauseUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The muteUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The closeUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The startUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The unmuteUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
#    And The renderUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-direct-HB-video-banner-1} 100% of the time
  ...
  Scenario: Send a web zone request from US and validate the response of a Programmatic Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {requestid=OX-VA-Render&unlimited=1} for zone named {zone-zoneset-NewRenderTest-web-programmatic-zone-video} to UAS
    Then The response code is 200
    And The response contains {var ut_aniview={publisherId:'5912cb1028a06142db67f38a',channelId:'5b7d30c4073ef454a07d4d64'}}
    And The response contains {VASTAdTagURI}
    And The response contains {ut_udms}
  ...
  Scenario: Send a web DT request and validate the response of a Programmatic Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with domain {bassfan.com&requestid=OX-VA-Render&tagid=430&optimize=1}
    And The response code is 200
    And The response contains {var ut_aniview={publisherId:\'5912cb1028a06142db67f38a\',channelId:\'5b7d30c4073ef454a07d4d64\'}}
    And The response contains {VASTAdTagURI}
    And The response contains {ut_udms}
  ...
  Scenario: Send a web HB request and validate the response of a Programmatic Video Adhesion Ad (With Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    Given i send 1 headerBidding secure post request for scenario {Send HB request for publisher 3708 with 320X50} for publisher 3708 with domain {cnn.com} with extra params {&requestid=OX-VA-Render&unlimited=1&optimize=1&sim_geo=1&country=us}
    And The response code is 200
    And The response contains {script}
...
#    And The firstQuartileUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The midpointUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The thirdQuartileUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The completeUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The pauseUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The muteUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The closeUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The startUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The unmuteUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
#    And The renderUrl has bannerid field matching the id of the banner named {campaign-NewRenderTest-web-programmatic-HB-video-banner-1} 100% of the time
    And The response contains {&e=firstQuartile}
    And The response contains {&e=midpoint}
    And The response contains {&e=thirdQuartile}
    And The response contains {&e=complete}
  ...
  Scenario: Send a web zone request from US and validate the response of a Direct Video Adhesion Ad (Without Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-VidAd-w/o-leave-behind-zone} to UAS
    Then The response code is 200
    And The response contains {var ut_aniview={publisherId:'5912cb1028a06142db67f38a',channelId:'5b7d30c4073ef454a07d4d64'}}
    And The response contains {VASTAdTagURI}
    And The response contains {ut_udms}
  ...
  Scenario: Send a web zone request from US and validate the response of a Programmatic Video Adhesion Ad (Without Leave Behind)
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {requestid=AN-VA-Render&unlimited=1} for zone named {zone-zoneset-NewRenderTest-VidAd-w/o-leave-behind-zone-P} to UAS
    Then The response code is 200
    And The response contains {var ut_aniview={publisherId:'5912cb1028a06142db67f38a',channelId:'5b7d30c4073ef454a07d4d64'}}
    And The response contains {VASTAdTagURI}
    And The response contains {ut_udms}
  ...
  Scenario: Send a web zone request from US and validate the response of a Direct Linear Video Ad
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-Instream-View-direct} to UAS
    Then The response code is 200
    And The response contains {VASTAdTagURI}
  ...
  Scenario: Send a web zone request from US and validate the response of a Programmatic Linear Video Ad
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {requestid=OX-CSVB-Render&unlimited=1} for zone named {zone-zoneset-NewRenderTest-Instream-View-programmatic} to UAS
    Then The response code is 200
    And The response contains {VASTAdTagURI}
    And The response contains {[CDATA[https://vast.doubleverify.com/v3/vast?}
    And The response contains {_vast=https%3A%2F%2Fnym1-ib.adnxs.com}
    ...
    And The response contains {https://tags.bluekai.com/site/15597?id=}
    And The response contains {https://idsync.rlcdn.com/403716.gif?partner_uid=}
    And The response contains {https://beacon.krxd.net/usermatch.gif?partner=undertone&partner_uid=}

  Scenario: Send a web zone request and validate the response of a direct ES banner with event trackers
    Given I add header of {x-forwarded-for} with value {207.246.116.162}
    When I send 1 times an ad request with parameter {unlimited=1} for zone named {zone-zoneset-NewRenderTest-Direct-event-trackers} to UAS
    Then The response code is 200
#    And The response contains {ut_scripts}
#    And The response contains {var ut_scripts = ["https://pixel.adsafeprotected.com/jload?anId=}
