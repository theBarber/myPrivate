...
#  UTID=a1b2c8faf42446dcbba3248cef1dc2bb is coded to 9kmivgho2hd1koqz149uu05ob
  ...
  Scenario: Seq-targeting for udmp, DT req, from app
    Given I clear all cookies from uas requests
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with extra params {udmp_seq_profile_inApp&deviceid=00000006-0006-0006-0006-000000000006&tagid=170}
    Then The response code is 200
    And The responses are passback
    Given I clear all cookies from uas requests
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with extra params {udmp_seq_profile_inApp&deviceid=60000006-6006-6006-6006-600000000006&tagid=170}
    Then The response code is 200
    And The responses are passback
    Given I clear all cookies from uas requests
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with extra params {udmp_seq_profile_inApp&deviceid=60000006-6006-6006-6006-600000000000&tagid=170}
    Then The response code is 200
    And The responses are passback
    Given I clear all cookies from uas requests
    Then i send 1 times Dynamic Tag ad request to UAS for publisher 3708 with extra params {udmp_seq_profile_inApp&deviceid=01000000-0100-0100-0100-010000000000&tagid=170}
    Then The response code is 200
    And The response contains script
...
#  UTID=a9b1c8faf27436dcbba3248cef1dc2bb is encoded to =a1o19osvf0w6rfwepq3avxy9n
  ...
  Scenario: PTzoneLimitation
    ...
    Then The response code is 200
    And The response contains script
    And The impressionUrl has bannerid field matching the id of the banner named {campaign-PTzoneLimitation-ST-1} 100% of the time
    Given I clear all cookies from uas requests
...
#####    Then i inject new profile doc with udId {21000000-0000-0000-0000-000000000000} on users bucket, where platform = {app}, profile type = {udmp_p}, profile num = 1992, and reduce 0 days from epoc time stamp and extra devices string = {"udid": "21000000-0000-0000-0000-000000000000"},{"udid": "22000000-0000-0000-0000-000000000000"},{"udid": "23000000-0000-0000-0000-000000000000"}
...
###    And The responses are passback
###    Given I clear all cookies from uas requests
###    When I send 1 times an ad request with parameter {deviceid=06060000-0060-0000-0000-000000006000&unlimited=1} for zone named {zone-zoneset-ProfileTargetingUDMPCrossDevice-ST-1} to UAS
###    Then The response code is 200
###    And The response contains script
###    And The impressionUrl has bannerid field matching the id of the banner named {campaign-ProfileTargetingUDMPCrossDevice-ST-1-banner-1} 100% of the time
###    Given I clear all cookies from uas requests
